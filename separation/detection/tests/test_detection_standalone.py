"""Regression tests for standalone detection replay and registration chaining."""
...
from separation.detection.binary import BinaryFile
from separation.detection.serialization import deserialize_object


PACKAGE_ROOT = Path(__file__).resolve().parents[1]
REGISTRATION_PACKAGE_ROOT = Path(__file__).resolve().parents[2] / "registration"
...
def test_detection_standalone_exact_replay():
    ...
    print(json.dumps(kwargs.get("settings", {}), indent=2, sort_keys=True, default=str))

    success = False
    try:
        with BlockSuite2PImports():
            standalone_module = importlib.import_module("separation.detection")
            actual_output = standalone_module.detection_wrapper(**kwargs)
...
def test_detection_compat_with_standalone_registration_outputs():
    _assert_no_suite2p_import_statements(PACKAGE_ROOT)
    _assert_no_suite2p_import_statements(REGISTRATION_PACKAGE_ROOT)

    captured_detection_input = _load_pickle(PACKAGE_ROOT / "test_input.pkl")
    captured_detection_output = _load_pickle(PACKAGE_ROOT / "test_output.pkl")
    captured_registration_input = _load_pickle(REGISTRATION_PACKAGE_ROOT / "test_input.pkl")

    opened: List[BinaryFile] = []
    detect_kwargs = deserialize_object(
        captured_detection_input,
        binary_file_factory=lambda d: _build_binary_file(d, opened),
    )
    reg_kwargs = deserialize_object(
        captured_registration_input,
        binary_file_factory=lambda d: _build_binary_file(d, opened),
    )
    expected_output = deserialize_object(captured_detection_output)

    # Explicit chaining: standalone registration output drives standalone detection params.
    badframes = np.array(reg_kwargs["badframes"], dtype=bool)

    success = False
    try:
        with BlockSuite2PImports():
            registration_module = importlib.import_module("separation.registration")
            detection_module = importlib.import_module("separation.detection")

            reg_outputs = registration_module.registration_wrapper(**reg_kwargs)

            chained_kwargs = dict(detect_kwargs)
            chained_kwargs["f_reg"] = reg_kwargs["f_reg"]
            chained_kwargs["yrange"] = reg_outputs["yrange"]
            chained_kwargs["xrange"] = reg_outputs["xrange"]
            chained_kwargs["badframes"] = np.logical_or(reg_outputs["badframes"], badframes)
            chained_kwargs["meanImg_chan2"] = reg_outputs.get("meanImg_chan2", None)

            actual_output = detection_module.detection_wrapper(**chained_kwargs)
    ...
    print(f"registration_to_detection_success={success}")
    assert success
